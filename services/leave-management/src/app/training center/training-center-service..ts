...
export class TrainingCenterService {
    ...
    constructor(
        private trainingCenterRepo: TrainingCenterRepository,
        private bookingsRepo: BookingsRepository
    ) { }

    async createMeetingRoom(req: TrainingCenterDto): Promise<CommonResponseModel> {
        try {

            const entity = new TrainingCenterEntity();
            ...
            const save = await this.trainingCenterRepo.save(entity);
            if (save) {
    ...
    async getAllMeetingRooms(): Promise<CommonResponseModel> {
        const result = await this.trainingCenterRepo.getAllTrainingCenterRepo()
        if (result.length > 0) {
    ...
    async getActiveMeetingRoom(): Promise<CommonResponseModel> {
        const data = await this.trainingCenterRepo.find({
            where: {
                isActive: true
            }
        });
    ...
    async updateMeetingRoom(req: TrainingCenterDto): Promise<CommonResponseModel> {
        try {
            const result = await this.trainingCenterRepo.update(
                { id: req.id },
                {
    ...
    async activateOrDeactivateMeetingRoom(req: TrainingCenterDto): Promise<CommonResponseModel> {
        try {
            const exists = await this.trainingCenterRepo.findOne({ where: { id: req.id } });
            if (!exists) {
                throw new CommonResponseModel(false, 99998, 'No meeting room found');
            }

            const update = await this.trainingCenterRepo.update(
                { id: req.id },
    ...
    async bookMeetingRoom(req: RoomBookingDto): Promise<CommonResponseModel> {
        ...
        const meetRoom = new TrainingCenterEntity()
        ...
        if (res.id) {
            // await this.trainingCenterRepo.update({ id: req.roomId }, { roomStatus: RoomStatusEnum.BOOKED })
        }
    ...
    async getAvailableRooms(): Promise<CommonResponseModel> {
        const data = await this.trainingCenterRepo.find({ where: { roomStatus: RoomStatusEnum.AVAILABLE } })
        if (data.length) {
    ...
    async imageUpload(filePath: string, filename: string, id: number, originalname: string): Promise<CommonResponseModel> {
        try {
            ...
            //     }
            //   }
            const filePathUpdate = await this.trainingCenterRepo.update(
                { id: id },
    ...
    async getTodayAvailabilitySlots(dto: SlotsDto): Promise<CommonResponseModel> {
        ...
        try {
            const room = await this.trainingCenterRepo.findOne({ where: { id: dto.roomId } });
            if (!room) {
    ...
    async getRoomSlots(req: DateTimeDto): Promise<CommonResponseModel> {
        ...
        const allRooms = await this.trainingCenterRepo.find({ where: { isActive: true } });
    ...
    async getAllBookingsForApprover(req: { id: number }): Promise<CommonResponseModel> {
        const approversMeetingRooms = await this.trainingCenterRepo.find({ where: { roomApproverId: req.id }, select: ['id'] })
