...
import { AlertMessages, BranchReq, TourClaimEnum, TourIntimationEnum } from "@hrexpert/shared-models";
...
import TourClaimDetailsForm from "./tour-claim-details-form";
...
const TourClaimView = () => {
    ...
    const submit = (values) => {
        try {
            tourIntimationService.gettourClaim(values).then((res) => {
                if (res.status) {
                    setData(res.data)
                    setfilterTableData(res.data.filter((rec) => rec.status === TourClaimEnum.OPEN))
    ...
    const dynamicColumns: ColumnsType<any> = [
        ...
        },
        {
            ...
            sorter: (a, b) => a.amountClaimed.localeCompare(b.amountClaimed),
            sortDirections: ['ascend', 'descend'],
            render: (text) => ('₹ ' + text + ' /-'),
        },
        {
            ...
            hidden: status === 'APPROVED' ? false : true,
            sorter: (a, b) => a.sanctionedAmount.localeCompare(b.sanctionedAmount),
            sortDirections: ['ascend', 'descend'],
            render: (text) => ('₹ ' + text + ' /-'),
        },
        {
            ...
            hidden: status === 'APPROVED' ? false : true,
            sorter: (a, b) => a.balanceAmount.localeCompare(b.balanceAmount),
            sortDirections: ['ascend', 'descend'],
            render: (text) => ('₹ ' + text + ' /-'),
        },
        {
            ...
            render: (text, record) => (
                <>
                    {(IAMClientAuthContext.user.roles === "SuperAdmin" ?
                        <> <Button style={{ marginTop: '1rem' }} type="primary" target='_blank' href={`http://139.59.79.77/hrexpert_dev_app/#/tour-claim-details-form?${record.employeeId}-${record.tourIntimationId}-${record.id}`}>
                            Approve
                        </Button> <Divider type="vertical" />
                            <Button style={{ marginTop: '1rem' }} color="danger" variant="solid" type="dashed" onClick={() => { setOpenModal(true), setrejectId(record.id) }}>Reject</Button></> : <></>)}
                    <Button style={{ marginTop: '5px' }} color="primary" variant="link" target='_blank' href={`${configVariables.TOUR_PDF_URL}TI-${record.id}-${record.employeeId}-TourClaim.pdf`}>Pdf</Button>
                </>
            )
        },
    ];
    ...
    const tabsOnchange = (value: string) => {
        setfilterTableData(data.filter((rec) => rec.status === value))
        setStatus(value)
    };

    const reject = (values) => {
        tourIntimationService.TourClaimReject(values).then((res) => {
            if (res.status) {
                AlertMessages.getSuccessMessage(res.internalMessage)
                setOpenModal(false)
    ...
    return (
        <>
            <PageContainer title="Tour Claim Applications" >
                ...
                {data.length > 0 && (
                    <Tabs defaultActiveKey={TourIntimationEnum.OPEN} onChange={tabsOnchange}>
                        {[
                            { label: <><SyncOutlined spin /> {TourIntimationEnum.OPEN} : <Tag color="blue">{data.filter((rec) => rec.status === TourIntimationEnum.OPEN).length}</Tag></>, key: TourIntimationEnum.OPEN },
                            { label: <><CheckCircleOutlined /> {TourIntimationEnum.APPROVED} : <Tag color="blue">{data.filter((rec) => rec.status === TourIntimationEnum.APPROVED).length}</Tag></>, key: TourIntimationEnum.APPROVED },
                            { label: <><ExclamationCircleOutlined /> {TourIntimationEnum.REJECTED} : <Tag color="blue">{data.filter((rec) => rec.status === TourIntimationEnum.REJECTED).length}</Tag></>, key: TourIntimationEnum.REJECTED },
                        ].map((tab) => (
                ...
                >
                    <Card>
                        <Form layout='vertical' form={form} onFinish={reject}>
                            <Row gutter={24}>
                                <Form.Item hidden name='id' initialValue={rejectId} >
                                    <Input defaultValue={rejectId} />
                                </Form.Item>
                                <Form.Item label='Remarks' name="remarks" style={{ margin: 0 }}>
                                    <TextArea placeholder="Enter Remarks" style={{ padding: '8px', width: '100%', minWidth: '700px', maxWidth: '700px', height: '50px' }} />
                                </Form.Item>
                            </Row>
                            <Row>
                                <Col xs={24} sm={12} md={8} lg={2} xl={2} style={{ marginTop: '23px' }}>
