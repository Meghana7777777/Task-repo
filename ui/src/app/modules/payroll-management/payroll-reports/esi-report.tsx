...
const PayrollEsiReport = (props: EsiReport) => {
    ...
    const columns: any = [
        ...
        },
        {
            ...
            render: (text) => (text ? text : '-'),
            align: "center",
            ...getColumnSearchProps('esicNo', 'ESI No')
        },
        {
            title: 'Employee Name',
        ...
        },
        {
            ...
            render: (text) => (text ? text : '-'),
            align: "center",
            ...getColumnSearchProps('payDays', 'Present Days')
        },
        {
            title: 'ESI Earnings',
            ...
            render: (text, record) => {
                ...
                return componentData['Total Earnings'] || '-'
            },
            align: "center",
        },
        {
            ...
            render: (text, record) => {
                const componentData = JSON.parse(record.componentRecords || '{}')
                return componentData['ESI-Employee'] || '-'
            },
            align: "center",
        },
        {
            ...
            render: (text, record) => {
                const componentData = JSON.parse(record.componentRecords || '{}')
                return componentData['ESI-Employer'] || '-'
            },
            align: "center",
        },
    ...
    const totalESIEmployee = data.reduce((sum, record) => {
        const componentData = JSON.parse(record.componentRecords || '{}');
        return sum + (parseFloat(componentData['ESI-Employee']) || 0);
    }, 0);

    const totalESIEmployer = data.reduce((sum, record) => {
        const componentData = JSON.parse(record.componentRecords || '{}');
        return sum + (parseFloat(componentData['ESI-Employer']) || 0);
    }, 0);

    const handleExportExcel = async () => {
        const workbook = new ExcelJS.Workbook();
        const worksheet = workbook.addWorksheet("ESI Report");
        ...
        for (let rowIdx = 0; rowIdx < rows.length; rowIdx++) {
            ...
            let colIndex = firstHeaderProcessed ? 2 : 1;
        ...
        const esiEmployeeCell = totalRow.getCell(8);
        esiEmployeeCell.value = totalESIEmployee;
        esiEmployeeCell.font = { bold: true };
        esiEmployeeCell.alignment = { vertical: "middle", horizontal: "center" };

        const esiEmployerCell = totalRow.getCell(9);
        esiEmployerCell.value = totalESIEmployer;
        esiEmployerCell.font = { bold: true };
        esiEmployerCell.alignment = { vertical: "middle", horizontal: "center" };
        ...
        saveAs(new Blob([buffer]), "ESI_Report.xlsx");
    };


    return (
        <Card title={'ESI Report'} extra={
            <Button style={{
                            ...
                            summary={() => (
                                <Table.Summary.Row>
                                    ...
                                    <Table.Summary.Cell index={1} align="center">
                                        <strong>{totalESIEmployee}</strong>
                                    </Table.Summary.Cell>
                                    <Table.Summary.Cell index={2} align="center">
                                        <strong>{totalESIEmployer}</strong>
                                    </Table.Summary.Cell>
                                </Table.Summary.Row>
                            )}
                        />
...
export default PayrollEsiReport