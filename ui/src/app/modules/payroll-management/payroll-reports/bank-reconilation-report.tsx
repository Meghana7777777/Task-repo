...
export const BankReconciliation = (props: any) => {
  ...
  const getBankReconilation = () => {
    ...
    form.validateFields().then(values => {
      ...
      service?.getBankRecompilationData(payload).then(res => {
        if (res.status) {
  ...
  const columns: any = [
    {
      ...
      render: (text, record) => {
        return record.bankName ? record.bankName : '-';
      },
    },
    {
  ...
  const modalColumns: any = [
    ...
    //   key: "bank",
    //   width: '5%',
    //   sorter: (a: { bank: string }, b: { bank: any }) =>
    //     a.bank?.localeCompare(b.bank),
    //   sortDirections: ['ascend', 'descend'],
    //   ...getModalColumnSearchProps('bank'),
    //   render: (text, record) => {
  ...
  const exportExcel = () => {
    ...
    saveAs(blob, `Bank-Reconciliation-Data${selectedDate}.xlsx`);
  };
  ...
  const exportEmpExcel = async () => {
    ...
    saveAs(blob, `Bank-Reconcilation-Employee-Data-${selectedDate}.xlsx`);
  };
...
.print-header {
  ...
  return (
    <Card title="Bank Reconciliation Report"
      ...
      >
        <Row gutter={24}>
          ...
          <Col xs={{ span: 24 }} sm={{ span: 24 }} md={{ span: 6 }} lg={{ span: 6 }} xl={{ span: 6 }} style={{ padding: '15px' }}>
            <Form.Item>
              <Button
                htmlType="submit" type='primary' onClick={() => getBankReconilation()} variant="outlined" color="primary">
                Submit
              </Button>

              <Button style={{ margin: 10, position: "relative" }}
        ...
        >
          <Card
            ...
            title={<span style={{ color: "black", width: '80%' }}>Bank Reconcilation Details </span>}
            ...
            <div id='modalContentId'>
              {selectedRecord && (
                <>
                  ...
                  <h3 style={{ textAlign: 'left', fontSize: '15px', fontWeight: '400px', color: "black" }} > Bank-Name: {modalBank}</h3>
