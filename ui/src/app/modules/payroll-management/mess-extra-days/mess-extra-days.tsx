...
import { DayWisePaySharedService, EmpRecCompSharedService } from "@hrexpert/shared-services";
...
export const MessExtraDaysUpload = () => {
    ...
    const handleFileChange = (event) => {
        ...
        const parseDate = (dateString: string): string => {
            return moment(dateString, ["DD-MM-YYYY", "DD/MM/YYYY"], true).isValid()
                ...
                : dateString; // Return original if not valid
        };
    ...
    const handleExport = (e: any) => {
        ...
        excel.saveAs(`Mess Extra Days-${currentDate}.xlsx`);
    }
    ...
    const handleUpload = () => {
        try {
            service.messExtraDaysExcel(data).then(res => {
                if (res.status) {
                    onReset()
    ...
    return (
        <Card
            title="Employee Mess Extra Days Upload"
            extra={<Button type="default" style={{ color: 'green' }} onClick={handleExport} icon={<FileExcelFilled />}> Sample Format</Button>}>
...
export default MessExtraDaysUpload