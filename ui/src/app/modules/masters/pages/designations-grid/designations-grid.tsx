...
import { DesignationsActivateDeactivateDto, ScopesEnum } from "@hrexpert/shared-models";
...
import DesignationsForm from "../designations-form/designations-form";
import { SequenceUtils } from "../../../../../app/common/utils";
import { Excel } from "antd-table-saveas-excel";



interface DesignationsViewIProps {
    scopes: ScopesEnum[]
}


const DesignationsView = (props:DesignationsViewIProps) => {
    const {scopes} =props
    ...
    const getDesignations = () => {
        try {
            service.getDesignations().then((res) => {
                if (res.status) {
                    setData(res.data)
                }
                else {
                    message.error("Failed To Retrieve Designations");
                }
            })
        } catch (error) {
            console.log(error);
    ...
    const deleteBranch = async (rowData: any) => {
        ...
        const req = new DesignationsActivateDeactivateDto(
            rowData.id,
            newIsActive,
            rowData.versionFlag
        );
        try {
            service.activateOrDeactivateDesignations(req).then((res) => {
                if (res.status) {
    ...
    let i = 1;
    const designations = [
        { title: "Name", dataIndex: "name", width: 400 }, // Set the width for the column
        { title: "Designation Code", dataIndex: "designationCode", width: 400 }, // Set the width for the column
    ];
    ...
    const exportExcel = () => {
        ...
        excel
            .addSheet('designations')
            .addColumns(designations) // Includes column widths
            .addDataSource(processedData, { str2num: false }) 
            .saveAs('Designations.xlsx');
    };
    ...
    const columns: ColumnsType<any> = [
        ...
        },
        {
            ...
            sorter: (a, b) => a.designationCode.localeCompare(b.designationCode),
            sortDirections: ['ascend', 'descend'],
            ...getColumnSearchProps("designationCode"),
            align: "center"
        },
        {
            title: "Status",
    ...
    return (
        <PageContainer breadcrumbRender={false} title={"Designations"}
            extra={
                <>
        ...
        >
            ...
            <Modal
                title={isUpdate ? "Update Designation" : "Create Designation"}
                open={modalVisible}
                onCancel={closeModal}
                footer={null}
                width="60%"
            ...
            >
                <DesignationsForm
                    key={Date.now()}
                    isUpdate={isUpdate}
                    designationsData={selectedStyleData}
