...
import QualificationsForm from "../qualifications-form/qualifications-form";
...
const QualificationsView = (props:QualificationsViewIProps) => {
    const {scopes} = props
    ...
    const getQualifications = () => {
        setLoading(true)
        try {
            service.getQualifications().then((res) => {
                if (res.status) {
                    setData(res.data)
                    setLoading(false)
                }
                else {
                    message.error("Failed To Retrieve Qualifications");
                }
            })
        } catch (error) {
            console.log(error);
    ...
    const updateQualifications = (data: QualificationsReq) => {
        service.updateQualifications(data)
            .then((res) => {
                if (res.status) {
            ...
            .catch((error) => {
                console.error("Error Updating Qualifications Details:", error);
            });
    };
    ...
    const deleteBranch = async (rowData: any) => {
        ...
        try {
            service.activateOrDeactivateQualifications(req).then((res) => {
                if (res.status) {
  ...
  const processedData = preprocessData(data); 
   excel
    ...
    return (
        <>
            <PageContainer title="Qualifications" breadcrumbRender={false}
                extra={
                    <>
            ...
            <Modal
                title={isUpdate ? "Update Qualifications" : "Qualifications"}
                open={modalVisible}
                onCancel={closeModal}
                footer={null}
                width="60%"
            ...
            >
                <QualificationsForm
                    key={Date.now()}
                    updateDetails={updateQualifications}
                    isUpdate={isUpdate}
                    qualificationsData={selectedStyleData}
