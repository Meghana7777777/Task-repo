...
import { CompanyActivateDeactivateReq, DivisionDto, ScopesEnum } from "@hrexpert/shared-models";
...
import CompanyForm from "../company-master-form/company-form";

interface CompanyGridIProps {
    scopes: ScopesEnum[]
}

const CompanyGrid = (props: CompanyGridIProps) => {
    const { scopes } = props
    ...
    const updateCompanyDetails = (data: DivisionDto) => {
    ...
    const deleteCompany = async (rowData: any) => {
        const newIsActive = !rowData.isActive;
        const req = new CompanyActivateDeactivateReq(
            rowData.id,
            newIsActive,
            rowData.versionFlag
        );
        try {
            service.activateDeactivateCompany(req).then((res) => {
                if (res.status) {
    ...
    const columns: ColumnsType<any> = [
        ...
        },
        {
            ...
            ...getColumnSearchProps("companyCode", "Company Code"),
            align: "center"
        },
        {
            title: "Status",
        ...
        },
        {
            ...
            render: (text, rowData) => (
                <span>
                    ...
                    <Popconfirm onConfirm={e => { deleteCompany(rowData); }}
                        title={
                            rowData.isActive
    ...
    return (
        <>
            <PageContainer title='Company' breadcrumbRender={false}
                extra={
                    <Space>
                        <Button
            ...
            >
                <CompanyForm
                    key={isUpdate ? selectedStyleData?.id : 'new-Company'}
                    updateDetails={updateCompanyDetails}
                    isUpdate={isUpdate}
                    companyData={selectedStyleData || {}}
