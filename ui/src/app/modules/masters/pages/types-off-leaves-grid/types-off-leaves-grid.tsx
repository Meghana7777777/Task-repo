...
import { ScopesEnum, TypeOfLeavesActivateDeactivateDto, TypeOfLeavesDto } from "@hrexpert/shared-models";
import { TypesOfLeavesService } from "@hrexpert/shared-services";
...
import TypesOfLeavesForm from "../types-off-leaves-form/types-off-leaves-form";
import { Excel } from "antd-table-saveas-excel";
interface TypesOfLeavesGridIProps {
    scopes: ScopesEnum[]
}

const TypesOfLeavesGrid = (props?:TypesOfLeavesGridIProps) => {
    ...
    const getAllTypesOfLeaves = () => {
        try {
            service.getAllTypesOfLeaves().then((res) => {
                if (res.status) {
                    setData(res.data)
                }
                else {
                    message.error("Failed to retrieve TypesOfLeaves");
                }
            })
        } catch (error) {
            console.log(error);
    ...
    const updateTypeOfLeavesDetails = (data: TypeOfLeavesDto) => {
        console.log(data, 'data');

        service.updateTypesOfLeaves(data)
            .then((res) => {
                if (res.status) {
                    message.success("TypesOfLeaves updated successfully");
            ...
            .catch((error) => {
                console.error("Error updating TypeOfLeavesDetails:", error);
            });
    };

    const deleteTypeOfLeaves = async (rowData: any) => {
        const newIsActive = !rowData.isActive;
        const req = new TypeOfLeavesActivateDeactivateDto(
            rowData.id,
            newIsActive,
            rowData.versionFlag
        );
        try {
            service.activateOrDeactivateTypesOfLeave(req).then((res) => {
                if (res.status) {
    ...
    const editTypeOfLeaves = (rowData: any) => {
    ...
    let i = 1;
    const TypesOfLeave = [
        { title: "Type Of Leave", dataIndex: "typeOfLeave"},
        { title: 'Leave Code', dataIndex: 'leaveCode' },
        { title: 'Default Leaves', dataIndex: 'defaultLeaves' },
    ];

    const preprocessData = (data) => {
        return data.map((record, index) => {
            const updatedRecord = { key: index + 1 }; 
            TypesOfLeave.forEach((column) => {
  ...
  const processedData = preprocessData(data); 
   excel
      .addSheet('TypesOfLeave')
      .addColumns(TypesOfLeave)
      .addDataSource(processedData, { str2num: false }) 
      .saveAs('Types Of Leave.xlsx');
  };
    ...
    const columns: ColumnsType<any> = [
        ...
        {
            title: "Default Leaves",
            dataIndex: "defaultLeaves",
            ...getColumnSearchProps("defaultLeaves"),
            align: "center"
        },
        {
            title: "Status",
                ...
                <span>
                    {rowData.isActive ? (
                        <EditOutlined
                            ...
                            onClick={() => {
                                if (rowData.isActive) {
                                    editTypeOfLeaves(rowData); // Call editBranch with selected row data
                                }
                            }}
                    ...
                    <Popconfirm
                        onConfirm={(e) => {
                            deleteTypeOfLeaves(rowData);
                        }}
                        title={
                            rowData.isActive
                                ? "Are you sure to Deactivate TypeOfLeaves ?"
                                : "Are you sure to Activate TypesOfLeaves?"
                        }
                    >
                        <Switch
                            size="default"
    ...
    return (
        <>
            <PageContainer title='Types Of Leaves' breadcrumbRender={false}
                extra={
                    <>
            ...
            >
                ...
                <TypesOfLeavesForm
                    ...
                    updateDetails={updateTypeOfLeavesDetails}
                    isUpdate={isUpdate}
                    leavesData={selectedStyleData}
                    closeForm={closeModal}
                    getAllTypesOfLeaves={getAllTypesOfLeaves}
                />
                </Modal>
...
export default TypesOfLeavesGrid;
