...
import IdProofForm from "./id-proof-form";
...
interface IdProofViewIProps {
    scopes: ScopesEnum[]
}


const IdProofView = (props:IdProofViewIProps) => {
    const {scopes} =props
    ...
    const service = new IdProofService();
    ...
    const getAllIdProofs = () => {
        try {
            service.getAllIdProofs().then((res) => {
                if (res.status) {
    ...
    const updateIdProof = (data: any) => {
        service.updateIdProof(data)
            .then((res) => {
                if (res.status) {
                    message.success("Id Proof updated successfully");
            ...
            .catch((error) => {
                console.error("Error updating Id Proof details:", error);
            });
    };

    const deleteIdProof = async (rowData: IdProofDto) => {
        rowData.isActive = !rowData.isActive;
        try {
            service.activateOrDeactivateIdProof(rowData).then((res) => {
                if (res.status) {
  ...
  const processedData = preprocessData(data); 
   excel
        ...
        },
        {
            ...
            render: (text, rowData) => (
                <span>
                    ...
                    <Popconfirm
                        onConfirm={(e) => {
                            deleteIdProof(rowData);
                        }}
                        title={
                            rowData.isActive
                                ? "Are you sure to Deactivate Id proof ?"
                                : "Are you sure to Activate Id Proof?"
                        }
                    >
                        <Switch
                            size="default"
    ...
    return (
        <>
            <PageContainer title='Id Proof' breadcrumbRender={false}
                extra={
                    <>
            ...
            <Modal
                title={isUpdate ? "Update Id Proof" : "Create  Id Proof"}
                open={modalVisible}
                onCancel={closeModal}
                footer={null}
                width="60%"
            ...
            >
                <IdProofForm
                    key={Date.now()}
                    updateDetails={updateIdProof}
                    isUpdate={isUpdate}
                    data={selectedStyleData}
                    closeForm={closeModal}
                    getAll={getAllIdProofs}
                />
