...
import { QualificationsActivateDeactivateDto, QualificationsReq, ScopesEnum } from "@hrexpert/shared-models";
...
import SpecializationsForm from "./specializations-form";



interface QualificationsViewIProps {
    scopes: ScopesEnum[]
}


const SpecializationsView = (props:QualificationsViewIProps) => {
    const {scopes} = props
    ...
    const getActiveQualifications = async () => {
        try {
            service.getActiveQualifications().then((res) => {
                if (res.status) {
    ...
    const getSpecializations = () => {
        setLoading(true)
        try {
            service.getSpecializations({}).then((res) => {
                if (res.status) {
                    setData(res.data)
                    setLoading(false)
                }
                else {
                    message.error("Failed To Retrieve Specializations");
                }
            })
        } catch (error) {
            console.log(error);
    ...
    const updateSpecializations = (data: QualificationsReq) => {
        service.updateSpecializations(data)
            .then((res) => {
                if (res.status) {
            ...
            .catch((error) => {
                console.error("Error Updating Specializations Details:", error);
            });
    };

    const deleteBranch = async (rowData: any) => {
        const newIsActive = !rowData.isActive;
        const req = new QualificationsActivateDeactivateDto(
            rowData.id,
            newIsActive,
            rowData.versionFlag
        );
        try {
            service.activateOrDeactivateSpecializations(req).then((res) => {
                if (res.status) {
    ...
    const qualification = [
        ...
        { title: "Qualification", dataIndex: "qualificationName"},
        { title: "Specializations", dataIndex: "specialization" },
    ];

    const preprocessData = (data) => {
        return data.map((record, index) => {
            const updatedRecord = { key: index + 1 }; 
            qualification.forEach((column) => {
  ...
  const processedData = preprocessData(data); 
   excel
      ...
      .addColumns(qualification)
      .addDataSource(processedData, { str2num: false }) 
      .saveAs('Specializations.xlsx');
  };
    ...
    const columns: ColumnsType<any> = [
        ...
        },
        {
            ...
            ...getColumnSearchProps("qualificationName"),
            align: "center"
        },
        {
    ...
    return (
        <>
            <PageContainer title="Specializations" breadcrumbRender={false}
                extra={
                    <>
            ...
            <Modal
                title={isUpdate ? "Update Specializations" : "Specializations"}
                open={modalVisible}
                onCancel={closeModal}
                footer={null}
                width="60%"
            ...
            >
                <SpecializationsForm 
                    key={Date.now()}
                    updateDetails={updateSpecializations}
                    isUpdate={isUpdate}
                    specializationsData={selectedStyleData}
