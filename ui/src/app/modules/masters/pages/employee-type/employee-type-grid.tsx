...
interface EmployeeTypeViewIProps {
    scopes: ScopesEnum[]
}


const EmployeeTypeView = (props:EmployeeTypeViewIProps) => {
    const {scopes}=props
    ...
    const getAllEmployeeTypes = () => {
        try {
            service.getAllEmployeeTypes().then((res) => {
                if (res.status) {
                    setData(res.data)
                }
                else {
    ...
    const updateEmplloyeeType = (data: any) => {
        service.updateEmplloyeeType(data)
            .then((res) => {
                if (res.status) {
                    message.success("Employee Type updated successfully");
            ...
            .catch((error) => {
                console.error("Error updating Employee Type details:", error);
            });
    };

    const deleteEmployeeType = async (rowData: any) => {
        rowData.isActive = !rowData.isActive;
        try {
            service.activateOrDeactivateEmployeetype(rowData).then((res) => {
                if (res.status) {
    ...
    const editEmployeeType = (rowData: any) => {
    ...
    let i = 1;
    const employeeType = [
        { title: "Employee Type", dataIndex: "name"},
    ];

    // const preprocessData = (data) => {
    //     return data.map((record, index) => {
    //         const updatedRecord = { key: index + 1 }; 
    //         employeeType.forEach((column) => {
  ...
  const processedData = preprocessData(data); 
   excel
      .addSheet('employeeType')
      .addColumns(employeeType)
      .addDataSource(processedData, { str2num: false }) 
      .saveAs('Employee Type.xlsx');
  };
        ...
        },
        {
            ...
            render: (text, rowData) => (
                <span>
                    ...
                    <Popconfirm
                        onConfirm={(e) => {
                            deleteEmployeeType(rowData);
                        }}
                        title={
                            rowData.isActive
                                ? "Are you sure to Deactivate Employee Type ?"
                                : "Are you sure to Activate Employee Type?"
                        }
                    >
                        <Switch
                            size="default"
    ...
    return (
        <>
            <PageContainer title='Employee Type' breadcrumbRender={false}
                extra={
                    <>
            ...
            <Modal
                title={isUpdate ? "Update Employee Type" : "Create  Employee Type"}
                open={modalVisible}
                onCancel={closeModal}
                footer={null}
                width="60%"
            ...
            >
                <EmployeeTypeForm
                    key={Date.now()}
                    updateDetails={updateEmplloyeeType}
                    isUpdate={isUpdate}
                    data={selectedStyleData}
                    closeForm={closeModal}
                    getAll={getAllEmployeeTypes}
                />
