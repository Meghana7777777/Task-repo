...
const LeaveBalanceYearlyReport = (props: LeaveAllocationViewProps) => {
    const { scopes } = props;
    ...
    const getEmpData = async () => {
        try {
            ...
            const req = new EmpDataReq(formValues.employeeId, null, null, dayjs(formValues.monthYear).format('YYYY01'), null,
                formValues.branchId === 'ALL' ? null : formValues.branchId, null, formValues.employeeTypeId,
            );

            const res = await leaveBalanceService.getAllLeaveBalanceAllocationsAllMonths(req);
    ...
    employees.forEach((employee) => {
        ...
        if (!existingEmployee) {
            ...
            leaveTypes.forEach((leave) => {
                for (let i = 1; i <= 12; i++) {
                    existingEmployee[`${leave.leaveTypeCode}_carried${i}`] = 0;
                    existingEmployee[`${leave.leaveTypeCode}_allocated${i}`] = 0;
                    existingEmployee[`${leave.leaveTypeCode}_opening${i}`] = 0;
                    existingEmployee[`${leave.leaveTypeCode}_utilized${i}`] = 0;
                    existingEmployee[`${leave.leaveTypeCode}_balance${i}`] = 0;
                }
            });
        ...
        if (employee.leaveTypeId) {
            ...
            if (leaveType) {
                for (let i = 1; i <= 12; i++) {
                    existingEmployee[`${leaveType.leaveTypeCode}_carried${i}`] = employee[`carried${i}`] || 0;
                    existingEmployee[`${leaveType.leaveTypeCode}_allocated${i}`] = employee[`accum${i}`] || 0;
                    existingEmployee[`${leaveType.leaveTypeCode}_opening${i}`] = (Number(employee[`carried${i}`]) + Number(employee[`accum${i}`])).toFixed(1) || 0;
                    existingEmployee[`${leaveType.leaveTypeCode}_utilized${i}`] = employee[`utilized${i}`] || 0;
                    existingEmployee[`${leaveType.leaveTypeCode}_balance${i}`] = ((Number(employee[`carried${i}`]) + Number(employee[`accum${i}`])) - (Number(employee[`utilized${i}`]))).toFixed(1) || 0;
                }
            }
        }
    });
    ...
    const monthColumns = Array.from({ length: 12 }, (_, index) => {
        const monthDate = new Date(2025, index);
        const monthName = monthDate.toLocaleString('en-US', { month: 'short' });

        return {
            title: `${monthName}'${currentYear} EMP LEAVE DETAILS`,
            children: [
                {
                    title: "Opening Leaves",
                    children: leaveTypes.map(leave => ({
                        title: leave.leaveTypeCode,
                        dataIndex: `${leave.leaveTypeCode}_opening${index + 1}`,
                        align: 'center',
                        key: `${leave.leaveTypeCode}_opening${index + 1}`,
                ...
                },
                {
                    ...
                    children: leaveTypes.map(leave => ({
                        title: leave.leaveTypeCode,
                        dataIndex: `${leave.leaveTypeCode}_utilized${index + 1}`,
                        align: 'center',
                        key: `${leave.leaveTypeCode}_utilized${index + 1}`,
                        render: (text) => text === 0 ? '0.0' : text,
                    }))
                },
                {
                    title: "Closing Leaves",
                    children: leaveTypes.map(leave => ({
                        title: leave.leaveTypeCode,
                        dataIndex: `${leave.leaveTypeCode}_balance${index + 1}`,
                        align: 'center',
                        key: `${leave.leaveTypeCode}_balance${index + 1}`,
                        render: (text) => text === 0 ? '0.0' : text,
                    }))
                }
            ]
        ...
        >
            <Form form={form} layout="vertical" onFinish={getEmpData}>
                ...
                <Row gutter={16} style={{ marginBottom: '10px', marginTop: '10px' }}>
                    ...
                    <Col>
                        <Button icon={<RedoOutlined />} type='dashed' onClick={onReset} danger>
                            Reset
                        </Button>
                    </Col>
...
export default LeaveBalanceYearlyReport;