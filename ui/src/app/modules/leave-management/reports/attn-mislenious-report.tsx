...
import '../../leave-management/attendance-adjustment/attendance-info.css';
import { PageContainer } from "@ant-design/pro-layout";


export default function MisleniousAttendanceReport() {
    ...
    const columns: ColumnsType<any> = [
        ...
        },
        {
            ...
            ...getColumnSearchProps("designation", "Desgination"),
            render: (text) => (text ? text : '-'),
        },
        {
            title: 'Attendance Status',
            dataIndex: 'attnStatus',
            ...getColumnSearchProps("attnStatus", "Attendance Status"),
            render: (text) => (text ? text : '-'),
        },
        {
            title: 'Date',
            ...
            sortDirections: ['ascend', 'descend'],
            ...getColumnSearchProps("attendanceDate", "Attendance Date"),
...

        {
            ...
            sorter: (a, b) => dayjs(a.totalWorkingHours).valueOf() - dayjs(b.totalWorkingHours).valueOf(),
            sortDirections: ['ascend', 'descend'],
            render: (text) => (text ? dayjs(text, 'HH:mm:ss').format('HH:mm') : '-')
        },
        {
            title: 'Less Working hours',
            dataIndex: 'lessWorkingHours',
            sorter: (a, b) => dayjs(a.lessWorkingHours).valueOf() - dayjs(b.lessWorkingHours).valueOf(),
            sortDirections: ['ascend', 'descend'],
            render: (text) => (text ? text : '-')
        },
        {
            title: 'Late Entry',
            dataIndex: 'lateEntry',
            ...getColumnSearchProps("lateEntry", "Late Entry"),
            render: (text) => (text ? text : '-')
        },
        {
            title: 'Early Exit',
            dataIndex: 'earlyExit',
            ...getColumnSearchProps("earlyExit", "Early Exit"),
            render: (text) => (text ? text : '-')
        },
    ...
    const getAllMissleniousData = () => {
        ...
        try {
            setLoading(true)
            service.getLateAndEarlyEntryEmployees(req).then((res) => {
                if (res.status) {
                    setData(res.data)
                    setShowColumns(true);
                    setLoading(false);
    ...
    const attendance = [
        ...
        {
            title: 'WorkingHours', dataIndex: 'totalWorkingHours',
            render: (text) => (text ? dayjs(text, 'HH:mm:ss').format('HH:mm') : '-')
        },
        { title: 'Less Working Hours', dataIndex: 'lessWorkingHours', },
        { title: 'Late Entry', dataIndex: 'lateEntry', },
        { title: 'Early Exit', dataIndex: 'earlyExit', },

    ];
    const exportExcel = () => {
        const excel = new Excel();
        excel
            .addSheet('attendance-miscellaneous-report')
            .addColumns(attendance)
            .addDataSource(data, { str2num: true })
            .saveAs('attendance-miscellaneous-report.xlsx');
    }


    return (
        <PageContainer title="Miscellaneous Attendance Report">
            <Form layout='vertical' form={form} initialValues={{ attendanceDate: defaultDateRange }}>
                <Row gutter={24}>
                    ...
                    <Col xs={24} sm={12} md={8} lg={2} xl={2} style={{ marginTop: "23px" }}>
                        <Button type='primary' onClick={getAllMissleniousData} variant="outlined" color="primary">Submit</Button>
                    </Col>

                    <Col xs={24} sm={12} md={8} lg={2} xl={2} style={{ marginTop: "23px" }}>
                        <Button type="dashed" icon={<UndoOutlined />} danger onClick={onReset} >Reset</Button>
                    </Col>
                    {data.length > 0 ? (
                        <>
                            <Col xs={24} sm={12} md={8} lg={2} xl={2} style={{ marginTop: "23px" }}>

                                <Button icon={<FileExcelOutlined />} className='panel_button' style={{ border: "1px dashed #22f534", color: "green", fontWeight: "bold" }} type="dashed" onClick={() => exportExcel()}>Get Excel</Button> </Col>
                        </>
                    ) : (<></>)}
