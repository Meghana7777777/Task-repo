...
const LateMinutesReport = () => {
    ...
    const getAllAttendance = () => {
        ...
        try {
            setLoading(true)
            service.getAllEmpLateMinutesData(req).then((res) => {
                if (res.status) {
                    setData(res.data)
                    setShowColumns(true);
                    setLoading(false);
    ...
    const generateColumns = () => {
        const dynamicColumns: ColumnsType<any> = [
            ...
            {
                title: 'Late Hours "<8 hrs"',
                dataIndex: 'late_hours',
                sorter: (a, b) => dayjs(a.late_hours).valueOf() - dayjs(b.late_hours).valueOf(),
                sortDirections: ['ascend', 'descend'],
    ...
    const exportExcel = () => {
        ...
        excel
            .addSheet('Less-Work-Hours-Report')
            .addColumns(adjustedColumns)
            .addDataSource(data, { str2num: true })
            .saveAs('Less-Work-Hours-Report.xlsx');
    }
    return (

        <PageContainer title="Less Work Hours Report">
...
export default LateMinutesReport