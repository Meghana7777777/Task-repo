...
export default function WorkerAttendanceMisReport() {
    ...
    const generateDynamicColumns = (data) => {
        ...
        const months = [...new Set(data.flatMap((item) =>
            Object.keys(item)
                .filter((key) => key.match(/plannedWorker(\d{6})|actualWorker(\d{6})/)) // Matches plannedWorkerMMYYYY / actualWorkerMMYYYY
        ...
        const dynamicColumns = months.map((month) => ({
            ...
            children: [
                {
                    title: "Planned",
                    dataIndex: `plannedWorker${month}`,
                    key: `planned${month}`,
                    align: "center",
                },
                {
                    title: "Actual",
                    dataIndex: `actualWorker${month}`,
                    key: `actual${month}`,
                    align: "center",
                },
                {
                    title: "Absent %",
                    dataIndex: `absentPercent${month}`,
                    key: `absent${month}`,
                    align: "center",
                    render: (_, record) => {
                        const planned = record[`plannedWorker${month}`] || 0;
                        const actual = record[`actualWorker${month}`] || 0;
                        const absentPercent = planned > 0 ? ((planned - actual) / planned) * 100 : 0;
                        return `${absentPercent.toFixed(2)}%`;
                    },
                },
            ],
        }));
    ...
    const exportToExcel = async () => {
        const fileName = `Worker-Attendance-Audit-Report.xlsx`;
        const workbook = new ExcelJS.Workbook();
        const worksheet = workbook.addWorksheet("Worker Audit");
    
        // **Step 1: Extract unique months from data keys**
        let availableMonths = new Set();
        data.forEach((row) => {
            Object.keys(row).forEach((key) => {
                const match = key.match(/(plannedWorker|actualWorker)(\d{6})/);
                if (match) {
        ...
        let headerRow1 = [
            ...
            "State",
            "Workers",
            ...sortedMonths.flatMap(() => ["", "", ""]), // For Planned, Actual, and Absent %
        ];
        let headerRow2 = [
            "",
            "",
        ...
        try {
            worksheet.mergeCells("D1:D2"); // Merge "Workers" header
    
            let colIndex = 5; // Starting column for months after Workers
        ...
        data.forEach((row, rowIndex) => {
            let rowData = [
                ...
                row.branchName || "-",
                row.state || "-",
                row.workerActive || "-", // Worker count or value
                ...sortedMonths.flatMap((month) => {
                    const planned = row[`plannedWorker${month}`] || 0;
                    const actual = row[`actualWorker${month}`] || 0;
                    const absentPercent = planned > 0 ? ((planned - actual) / planned) * 100 : 0;
                    return [
                        planned,
                        actual,
                        `${absentPercent.toFixed(2)}%`, // Format as percentage
                    ];
                }),
            ];
    ...
    return (
        <>
            <Card title='Worker Attendance MIS Report'
                extra={<span><Button
