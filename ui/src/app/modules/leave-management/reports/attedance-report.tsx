import { FileExcelOutlined, PieChartOutlined, SearchOutlined, UndoOutlined } from '@ant-design/icons';
...
import { Badge, Button, Card, Col, DatePicker, Form, Input, message, Modal, Row, Select, Space, Table, Tag } from 'antd';
...
import AttendancePieChart from '../attendance-adjustment/attendance-pieChart';
...
const AttendanceReport = () => {
    ...
    const getAllAttendance = () => {
        ...
        if (formValues.attendanceDate) {
            req.attnFromDate = formValues.attendanceDate?.format('YYYY-MM-DD');
            req.attnToDate = formValues.attendanceDate?.format('YYYY-MM-DD');
        }
    ...
    //         const presentCount = records.filter((r: any) => r.attnStatus === "P").length;
    //         const absentCount = records.filter((r: any) => r.attnStatus === "A").length;
    //         const leaveCount = records.filter((r: any) => r.presentStatus !== "A").length;
    //         return {
    ...
    const mainColumns: ColumnsType<any> = [
        ...
        },
        {
            ...
            dataIndex: 'empCode',
            width: '9rem',
            ...getColumnSearchProps('empCode', 'Employee Id'),
        },
        // {
        //     title: 'Branch',
        //     dataIndex: 'branches',
        ...
        // },
        {
            ...
            ...getColumnSearchProps('empCode', 'Attendance Date'),
            render: (text: any) =>
        ...
        },
        {
            title: 'Shift',
            dataIndex: 'shift',
            ...getColumnSearchProps('empCode', 'Shift'),
            render: (text: any) =>
                text ? 'G' : '-',
        },
        {
            ...
            ...getColumnSearchProps('empCode', 'Attendance Status'),
        ...
        },
        {
            ...
            ...getColumnSearchProps('empCode', 'Leave Status'),
            render: (text: any) =>
                text === 'A' ? '-' : text,
        },
        {
    ...
    return (
        <>
            ...
            <PageContainer title="Attendance Report">
                <Form layout='vertical' form={form} initialValues={{ attendanceDate: dayjs() }}>
                    <Row gutter={24}>
                        ...
                        <Col xs={24} sm={12} md={8} lg={2} xl={2} style={{ marginTop: "23px" }}>
                            <Button type="primary" onClick={getAllAttendance} variant="outlined" color="primary">
                                Submit
                            </Button>
                        </Col>

                        {data.length > 0 ? <>  <Col xs={24} sm={12} md={8} lg={2} xl={2} style={{ marginTop: "23px" }}>
                            <Button color='primary' icon={<PieChartOutlined />} onClick={() => setIsModalOpen(true)} >
                                statistics
                            </Button>
                        </Col></> : <></>}
                ...
                {data.length > 0 ? <>
                    <Row className="row" style={{ justifyContent: 'flex-end' }}>
                        <Tag color={'blue'} style={{ fontSize: '16px', padding: '6px 12px' }}>
                            {`${'Employees'}: ${data.length}`}
                        </Tag>
                        <Tag color={'green'} style={{ fontSize: '16px', padding: '6px 12px' }}>
                            {`${'Present'}: ${data.filter((r: any) => r.attnStatus !== "A" && r.attnStatus !== "H" && r.attnStatus !== "W").length}`}
                        </Tag>
                        <Tag color={'red'} style={{ fontSize: '16px', padding: '6px 12px' }}>
                            {`${'Absent'}: ${data.filter((r: any) => r.attnStatus === "A" || r.attnStatus === "H" || r.attnStatus === "W").length}`}
                        </Tag>
                        <Tag color={'geekblue'} style={{ fontSize: '16px', padding: '6px 12px' }}>
                            {`${'Leaves'}: ${data.filter((r: any) => r.leaveStatus !== "A").length}`}
                        </Tag>
                    </Row>
                </> : <></>}
                <br></br>
            ...
            <Modal width={800} footer={null} open={isModalOpen} onCancel={() => setIsModalOpen(false)}>
                <AttendancePieChart data={data} branch={form.getFieldValue('branches') === 'ALL' ? 'ALL' : branches?.find((res) => res.id === form.getFieldValue('branches'))?.branchName} />
            </Modal>
