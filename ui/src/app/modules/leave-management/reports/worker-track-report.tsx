...
export default function BranchWiseWorkerTrack() {
    ...
    function getReportData() {
        ...
        empService.getBranchWiseWorkerStatusReport(req).then((res) => {
            if (res.status) {
                setData(res.data)
            } else {
                setData([])
    ...
    const columns: any = [
        ...
        },
        {
            ...
            children: [
                {
                    title: 'Present',
                    dataIndex: 'workerPresentMTD',
                    key: 'workerPresentMTD',
                    align: 'center'
                },
                {
                    title: 'Absent',
                    dataIndex: 'workerAbsentMTD',
                    key: 'workerAbsentMTD',
                    align: 'center'
                },
                {
                    title: 'Leave',
                    dataIndex: 'workerLeaveMTD',
                    key: 'workerLeaveMTD',
                    align: 'center'
                }
            ]
        },
        {
            ...
            children: [
                {
                    title: 'Cash',
                    dataIndex: 'workerCashPayment'
                },
                {
                    title: 'Bank',
                    dataIndex: 'workerBankPayment'
                }
            ]
        },
        {
    ...
    const exportToExcel = async () => {
        ...
        const worksheet = workbook.addWorksheet("Worker Attendance Audit");

        // Define column headers and structure based on the provided table
        const headers = [
            ["Branch Name", "Worker", "", "Attendance (MTD)", "", "", "Payroll", "", "Leave Policy Code", "Salary Policy Code"],
        ...
        worksheet.mergeCells("B1:C1"); // Worker
        ...
        data.forEach(row => {
            const excelRow = [
                ...
                row.workerActive, row.workerInactive,
                row.workerPresentMTD, row.workerAbsentMTD, row.workerLeaveMTD,
                row.workerCashPayment, row.workerBankPayment,
    ...
    return (
        <>
            <Card title='Worker Attendance Audit Report'
                extra={<span><Button
