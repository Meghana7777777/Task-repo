...
import { Badge, Button, Card, Col, DatePicker, Drawer, Form, Input, List, message, Popconfirm, Row, Select, Space, Table, Tag, Typography } from 'antd';
...
const AttendanceInfo = () => {
    ...
    const groupDataByEmployee = (data: any[]) => {
        ...
        return Object.keys(grouped).map((key) => {
            ...
            const presentCount = records.filter((r: any) => r.attnStatus !== "A" && r.attnStatus !== "H" && r.attnStatus !== "W").length;
            const absentCount = records.filter((r: any) => r.attnStatus === "A" || r.attnStatus === "H" || r.attnStatus === "W").length;
            const leaveCount = records.filter((r: any) => r.leaveStatus !== "A").length;
            return {
