...
import { Badge, Button, Col, DatePicker, Drawer, Form, Input, message, Row, Select, Space, Table, Tag, Typography } from 'antd';
...
import LateMinMomentRecords from '../late-min-moment-records/late-min-moment-rec';
import LateMinRecords from './late-min-records';

interface AttendanceEmployeeInfoProps {
    scopes: ScopesEnum[]
}

const AttendanceEmployeeInfo = (props: AttendanceEmployeeInfoProps) => {
    ...
    const [openLateMinDrawer, setOpenLateMinDrawer] = useState(false);
    const [modelData, setModelData] = useState<any>();
    const [employeeTypes, setEmployeeTypes] = useState([])
    const employeeTypesService = new EmployeeTypeService()
    const [selectedEmpLatMin, setSelectedEmpLatMin] = useState<string>();
    const [selectedEmpLatMinDate, setSelectedEmpLatMinDate] = useState<string>();
    ...
    const detailColumns: ColumnsType<any> = [
        ...
        },
        {
            ...
            render: (text: any, record: any) => {
                if (text === '00:00:00') return '-';
        
                const [hours, minutes, seconds] = text.split(':').map(Number);
                const totalMinutes = hours * 60 + minutes;  
        
                const finalMinutes = Math.max(totalMinutes - record.momentsLateMin, 0)
                ...
                return (
                    <>
                        {`${finalHours.toString().padStart(2, '0')}:${finalMins.toString().padStart(2, '0')}`}
                    </>
                );
            },
        },
        {
            ...
            render: (text: any, record: any) => {
                return (
                    <>
                        {text ?
                            <Typography.Link onClick={() => { setOpenLateMinDrawer(true), setSelectedEmpLatMinDate(record.attendanceDate), setSelectedEmpLatMin(record.empCode) }}>
                                {text}
                            </Typography.Link>
                            : '-'}
                    </>
            ...
            hidden: branches?.find((res) => res.id === Number(form.getFieldValue('branches')))?.branchName?.includes("CORPORATE") ? false : true
        },
        // {
    ...
    const attendance = [
        ...
        //s { title: 'Late min per month', dataIndex: 'cumLateMin', render: (text) => (text ? text : '-') },
    ...
    const exportExcel = () => {
        ...
        console.log(groupedData, "datadatadata")
    ...
    const onLateminDrawerClose = () => {
        setOpenLateMinDrawer(false);
    };
    ...
    return (
        <>
            ...
            <Drawer
                //  title={` Drawer`}
                placement="right"
                onClose={onLateminDrawerClose}
                open={openLateMinDrawer}
                size={'large'}
                width="65%"

            >
                <LateMinRecords scopes={props.scopes} employeeCode={selectedEmpLatMin} date={selectedEmpLatMinDate} getAllAttendance={getAllAttendance} />
            </Drawer>
...
export default AttendanceEmployeeInfo