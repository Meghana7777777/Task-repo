...
export function LeavesApplyManualForm(
    ...
    const [latestFreezeDate, setLatestFreezeDate] = useState<any>('2024-12-31');
    ...
    const getAllLeaveAllocationsData = (values) => {
        ...
        const req = new EmpDataReq(formValues.employeeId, null, null, dayjs().format('YYYYMM'), null,
            formValues.branchId === 'ALL' ? null : formValues.branchId, null, 1, null, formValues.employeeCode
        );
        try {
            leaveAllocationservice.getAllNewLeaveAllocationsLeaveTypes(req).then((res) => {
                if (res.status) {
                    setLeaveAllocations(res.data)
                    setLatestFreezeDate(res.data[0].latestFreezeDate)
    ...
    console.log(latestFreezeDate, '-------------latestFreezeDate-----------')
    ...
    const disablePastAndFreezeDate = (current) => {
        if (!latestFreezeDate) return false; 
        return current && current.isSameOrBefore(dayjs(latestFreezeDate), 'day');
    };
    ...
    const updateLeaveAllocations = (req: any) => {
        leaveAllocationservice.updateNewLeaveAllocations(req).then((res) => {
            if (res.status) {
            }
        })
    }
    ...
    return (
        <>
            <PageContainer title='Employee Leave Application' breadcrumbRender={false}
                extra={
                    <>
            ...
            >
                ...
                >
                    ...
                    <Row gutter={[24, 4]}>
                        ...
                        {/* <Col xs={24} sm={12} md={8} lg={6} xl={6}> */}
                        <Form.Item hidden label={'Employee Type'} name='employeeTypeId'>
                        ...
                        <Col xs={24} sm={12} md={8} lg={6} xl={6}>
                            <Form.Item
                                key={Date.now()}
                                name="typeOfLeave"
                                // label={<span>Type of Leave 
                                //     {/* || Available Leaves: <Tag color="green">{availableLeaves}</Tag> */}
                                //     {/* <Tag color="orange">Minimum Limit:{minLeaves}</Tag>
                                //     <Tag color="blue">Maximum Limit:{maxLeaves}</Tag> */}
                                //     {leaveAllocations?.map((res) => (
                                //         <Tag key={res.id} color={leaveColors[res.leaveCode.trim()] || "default"}>
                                //             {res.leaveCode.trim()} : {res.available}
                                //         </Tag>
                    ...
                    <Row>
                        <Col span={24} style={{ textAlign: 'right' }}>
                            <Button htmlType="submit" className="panel_button" color="primary" variant="outlined" >
                                Submit
                            </Button>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                            <Button htmlType="button" onClick={onReset} icon={<UndoOutlined />} type='dashed' danger>
                                Reset
                            </Button>
                        </Col>
                    </Row>
