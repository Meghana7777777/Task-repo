...
import SelfAttendanceAdjustmentView from './self-attendance-adjustment-view';
...
const { Option } = Select;
const SelfAttendanceAdjustment = (props: AttendanceAdjustmentProps) => {
  ...
  useEffect(() => {
    if (props.isUpdate) {
      form.setFieldsValue({
        ...
        date: dayjs(props?.Data?.attnAdjstDate),
        shift: props?.Data?.shift,
        presentStatus: props?.Data?.presentStatus,
        inTime: dayjs(props?.Data?.inTime),
        outTime: dayjs(props?.Data?.outTime),
    ...
    try {
      ...
      service.getActiveEmployeesByIds(req).then((res) => {
        ...
        if (res.status) {
          ...
          const data = res.data.data && res.data.data.find(item => item.id === Number(IAMClientAuthContext.user?.employeeId));
          form.setFieldsValue({
  ...
  const handleSelectEmployeeName = () => {
    const data = employeeData && employeeData.find((item) => item.id === Number(IAMClientAuthContext.user?.employeeId));
    console.log(data)
    form.setFieldsValue({
  ...
  const getAdjustmentData = () => {
    const empId = IAMClientAuthContext.user?.employeeCode
    const currDate = new Date();
    const date = dayjs(form.getFieldValue('date')).format('YYYY-MM-DD') || dayjs(currDate).format('YYYY-MM-DD');
    const request = new AttendanceAdjustRequest(empId, date);
  ...
  const saveData = (req: AttnAdjustLogReq) => {
    ...
    formData.append('empId', IAMClientAuthContext.user?.employeeId);
    formData.append('employeeCode', IAMClientAuthContext.user?.employeeCode);
    formData.append('branchId', IAMClientAuthContext.user?.unitId);
    ...
    attnService.attendanceAdjustment(formData).then(res => {
      if (res.status) {
        onReset();
        AlertMessages.getSuccessMessage(`Attendance Adjustment ${props.isUpdate ? 'Updated' : 'Applied'} Successfully`);
        props.isUpdate ?? props.closeForm
      } else {
  ...
  return (
    <PageContainer title={<span >Attendance Adjustment</span>}
    ...
    >
      ...
      {(props.isUpdate ? <></> : <><SelfAttendanceAdjustmentView /></>)}
    </PageContainer>
  )

}
export default SelfAttendanceAdjustment
